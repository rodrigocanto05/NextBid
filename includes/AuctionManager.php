...
class AuctionManager {
    ...
    /**
     * Adiciona imagem usando a coluna img_prd_id
     */
    public function addImage(int $productId, string $path): bool {
        $sql = "INSERT INTO product_images (img_path, img_prd_id) VALUES (?, ?)";
    ...
    public function getActiveAuctions(?float $userLat = null, ?float $userLng = null, int $raio = 50): array {
        ...
        $sql = "SELECT p.*, c.cat_name, MIN(i.img_path) as main_image $geoQuery
                FROM products p
                INNER JOIN categories c ON p.prd_cat_id = c.cat_id
                LEFT JOIN product_images i ON p.prd_id = i.img_prd_id
