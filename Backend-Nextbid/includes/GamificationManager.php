...
class GamificationManager {
   ...
   //ISto é da caça ao tesouro ainda toua fazer
    public function joinHunt(int $userId, int $pointId): array {
        ...
        $stmt = $this->pdo->prepare("SELECT gpart_id FROM gamification_participants WHERE gme_id = ? AND usr_id = ?");
        $stmt->execute([$pointId, $userId]);
        if ($stmt->fetch()) return ['status' => 'error', 'message' => 'Já estás inscrito nesta caça!'];

        $this->pdo->beginTransaction();
        try {
            $stmt = $this->pdo->prepare("INSERT INTO gamification_participants (gme_id, usr_id) VALUES (?, ?)");
            $stmt->execute([$pointId, $userId]);
    ...
    public function claimPointWithCode(int $userId, int $pointId, float $userLat, float $userLng, string $inputCode): array {
        ...
        $this->pdo->beginTransaction();
        try {
            $this->pdo->prepare("UPDATE gamification SET gme_status = 'collected' WHERE gme_id = ?")->execute([$pointId]);

            $this->pdo->prepare("INSERT INTO gamification_claims (gme_id, usr_id) VALUES (?, ?)")->execute([$pointId, $userId]);

            $this->pdo->prepare("INSERT INTO xp_logs (usr_id, xpl_amount, xpl_reason) VALUES (?, ?, ?)")
